...
from emergent_specialization.reporting.analysis import checkpoint_rows, hse_trajectory_rows, load_run
from emergent_specialization.runtime.batch import nominal_call_counts
from emergent_specialization.core.config import RunConfig, load_config
from emergent_specialization.runtime.health import run_health
from emergent_specialization.core.logging import git_commit
...
def _run_one_row(
    ...
    *,
    ...
    if row.get("run_dir") and row.get("status") == "incomplete":
        command = [sys.executable, "-m", "emergent_specialization.runtime.experiment", "--resume", str(row["run_dir"])]
    else:
        command = [sys.executable, "-m", "emergent_specialization.runtime.experiment", "--config", str(_resolve(row["config_path"])), "--seed", str(row["seed"]), "--output-dir", str(run_root)]
    if mock:
