...
def _execute(manifest: dict[str, Any]) -> int:
    ...
    for raw in manifest["runs"]:
        ...
        if found is not None and raw.get("status") == "incomplete":
            run_dir, _ = found
            command = [sys.executable, "-m", "emergent_specialization.runtime.experiment", "--resume", str(run_dir), "--confirm-real"]
        else:
            command = [sys.executable, "-m", "emergent_specialization.runtime.experiment", "--config", row.config_path, "--seed", str(row.seed), "--confirm-real"]
