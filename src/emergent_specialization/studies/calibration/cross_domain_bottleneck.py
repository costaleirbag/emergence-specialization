...
from emergent_specialization.studies.ecology.ecological_information import FAMILIES, GEOMETRIES, V3Case, generate_environment, solve
from emergent_specialization.studies.ecology.ecological_information_v31 import SEMANTIC_SCHEMAS
...
def freeze_manifest() -> dict[str, Any]:
    ...
    prompt_audit = {"protocol": PROTOCOL, "answer_leak": any(bool(t["answer_leak"]) for t in tasks), "geometry_names_in_prompts": any(any(name in render_user(t) for name in GEOMETRIES) for t in tasks), "counts_by_arm": dict(counts), "representation_modes": {arm: sorted({t["representation_mode"] for t in tasks if t["arm"] == arm}) for arm in ARMS}, "explicit_rule_arms": sorted({t["arm"] for t in tasks if t["explicit_rule"]}), "static_instruction_hash": STATIC_INSTRUCTION_HASH}
    v2.atomic_json(REPORT_ROOT / "prompt_audit.json", prompt_audit)
