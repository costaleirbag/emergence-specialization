...
from emergent_specialization.studies.theory.v1.micro_design import ECOLOGIES, MACRO_CHECKPOINTS, MACRO_ROUNDS, N_NICHES, PROBES_PER_NICHE, SOCIAL_SEEDS, macro_cells, stable_hash
from emergent_specialization.studies.theory.v1.micro_runner import INPUT_PRICE, CACHED_INPUT_PRICE, OUTPUT_PRICE, MODEL, THINKING, MAX_TOKENS, REPORT_ROOT, DATA_ROOT, SYSTEM_PROMPT, OUTPUT_INSTRUCTION
...
class RequestGate:
    """One global semaphore plus offline-safe request-overlap telemetry.

    The gate is shared by t0, online trajectory steps, and checkpoint probes.
    Scientific state never crosses this boundary; only transport capacity does.
    """
...
async def _mock_protocol_trajectory(trajectory_id: int, gate: RequestGate, out: list[dict[str, Any]]) -> None:
    """Tiny deterministic protocol used only by offline concurrency tests."""
    memory: list[int] = []
    posterior = [0, 0, 0, 0]
    trajectory_events: list[dict[str, Any]] = []
    for t in range(1, 5):
        async with gate:
            await asyncio.sleep((trajectory_id + t) % 3 * 0.0001)
            response = (trajectory_id * 7 + t) % 2
        selected = (trajectory_id + t + response) % 4
        posterior[selected] += int(response == 1)
        memory.append(response)
        trajectory_events.append({"t": t, "selected": selected, "response": response, "memory": tuple(memory), "posterior": tuple(posterior)})
        if t in (2, 4):
            snapshot = tuple(memory)
            probes = []
            for probe in range(4):
                async with gate:
                    await asyncio.sleep((probe + trajectory_id) % 2 * 0.0001)
                    probes.append((probe, (trajectory_id + t + probe + sum(snapshot)) % 2))
            if tuple(memory) != snapshot or tuple(posterior) != trajectory_events[-1]["posterior"]:
                raise AssertionError("mock checkpoint mutated trajectory state")
            trajectory_events.append({"checkpoint": t, "probes": tuple(probes), "memory": tuple(memory), "posterior": tuple(posterior)})
    out.append({"trajectory": trajectory_id, "events": trajectory_events, "final_memory": tuple(memory), "final_posterior": tuple(posterior)})


async def run_mock_protocol(concurrency: int = 1) -> dict[str, Any]:
    """Run a complete miniature t0+trajectory+checkpoint protocol offline."""
    gate = RequestGate(concurrency)
    t0: list[tuple[int, int]] = []
    async def t0_call(i: int) -> None:
        async with gate:
            await asyncio.sleep((i % 3) * 0.0001)
            t0.append((i, i % 2))
    await asyncio.gather(*[t0_call(i) for i in range(8)])
    trajectories: list[dict[str, Any]] = []
    await asyncio.gather(*[_mock_protocol_trajectory(i, gate, trajectories) for i in range(4)])
    return {"t0": tuple(sorted(t0)), "trajectories": tuple(sorted(trajectories, key=lambda x: x["trajectory"])), "max_active": gate.max_active, "completed": gate.completed}
    ...
    for agent in range(NUM_AGENTS):
        for niche in range(N_NICHES):
            for probe_index in range(PROBES_PER_NICHE):
                task = _probe(ecology, seed, spec, niche, probe_index, checkpoint)
                lid = _lid("checkpoint", ecology, seed, str(cell["cell_id"]), checkpoint, agent, niche, probe_index)
                if lid not in terminal:
                    pending.append((lid, agent, task, copy.deepcopy(snapshot[agent])))
    results = await asyncio.gather(*[
        _completion(backend, task=task, ecology=ecology, logical_id=lid, memory=memory,
                    events_path=events_path, semaphore=gate, attempts=attempts,
                    append_lock=append_lock)
        for lid, agent, task, memory in pending
    ])
...
async def _run_trajectory(*, backend: DeepSeekDirectBackend, ecology: str, seed: int, cell: dict[str, Any], spec: dict[str, Any], state: State, terminal: dict[str, Any], prior_steps: dict[int, dict[str, Any]], attempts: dict[str, int], events_path: Path, checkpoint_path: Path, gate: RequestGate, append_lock: asyncio.Lock) -> None:
    ...
    for t, task in enumerate(spec["online"], 1):
        ...
        chosen = terminal[lid] if lid in terminal else await _completion(
            backend, task=task, ecology=ecology, logical_id=lid,
            memory=copy.deepcopy(state.memories[selected]), events_path=events_path,
            semaphore=gate, attempts=attempts, append_lock=append_lock,
        )
        terminal[lid] = chosen
...
async def run_macro(*, confirm_real: bool = False, concurrency: int = CONCURRENCY) -> dict[str, Any]:
    ...
    try:
        ...
        # All t0 tasks are read-only and independent, so they are gathered under
        # the same global request gate used by trajectories and checkpoints.
        t0_pending = []
                ...
                for agent in range(NUM_AGENTS):
                    for niche in range(N_NICHES):
                        for probe_index in range(PROBES_PER_NICHE):
                            ...
                            if lid not in terminal:
                                t0_pending.append((lid, ecology, task))
        t0_results = await asyncio.gather(*[
            _completion(backend, task=task, ecology=ecology, logical_id=lid,
                        memory=[], events_path=events_path, semaphore=gate, attempts=attempts, append_lock=append_lock)
            for lid, ecology, task in t0_pending
        ])
        # The ecology is encoded in the task's seed specification, not safely
        # inferred from a hash. Reconstruct it by matching the frozen IDs.
        for (lid, ecology, task), result in zip(t0_pending, t0_results):
            if lid not in terminal:
                # completion already validated the model; only the map update is
        ...
        await asyncio.gather(*trajectory_jobs)
        ...
        status.update(status="completed", run_id=run_id, logical_calls=len(final_terminal), physical_attempts=len([e for e in final_events if e.get("event") == "completion"]), retries=sum(int(e.get("attempt", 0)) for e in final_events if e.get("event") == "completion"), observed_cost_usd=sum(float(e.get("attempt_cost_usd") or 0.0) for e in final_events if e.get("event") == "completion"), global_request_limit=gate.limit, max_active_requests=gate.max_active, completed_requests=gate.completed, finished_at_utc=now())
    except Exception as exc:
        status.update(status="incomplete", run_id=run_id, failure=f"{type(exc).__name__}: {exc}", global_request_limit=gate.limit, max_active_requests=gate.max_active, completed_requests=gate.completed, finished_at_utc=now()); raise
    finally:
...
def health() -> dict[str, Any]:
    ...
    status_path = MACRO_ROOT / "macro_status.json"
    ...
    models = sorted({model for event in events for model in [(event.get("provider_metadata") or {}).get("model")] if isinstance(model, str) and model})
    return {"protocol": PROTOCOL, "run_id": status.get("run_id"), "logical_expected": manifest["logical_calls"], "logical_terminal": len(terminal), "physical_attempts": len(events), "technical_retries": sum(int(e.get("attempt", 0)) for e in events), "error_categories": dict(errors), "coverage": len(terminal) / manifest["logical_calls"], "observed_cost_usd": cost, "models": models, "global_request_limit": status.get("global_request_limit", CONCURRENCY), "max_active_requests": status.get("max_active_requests"), "completed_requests": status.get("completed_requests"), "classification": "CLEAN" if len(terminal) == manifest["logical_calls"] and not any(int(e.get("attempt", 0)) for e in events) else ("COMPLETE_WITH_RETRIES" if len(terminal) == manifest["logical_calls"] else "INCOMPLETE")}
