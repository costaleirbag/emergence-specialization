...
def analyze() -> dict[str, Any]:
    ...
    completion_events = [e for e in events if e.get("event") == "completion" and e.get("terminal")]
    ...
    technical = {"logical_calls": manifest["logical_calls"], "terminal_completions": len(terminals), "physical_attempts": len(completion_events), "retries": sum(int(e.get("attempt", 0)) for e in completion_events), "semantic_ood": sum(int(bool(e.get("semantic_ood"))) for e in completion_events), "models": sorted({(e.get("provider_metadata") or {}).get("model") for e in completion_events}), "fingerprints": sorted({(e.get("provider_metadata") or {}).get("system_fingerprint") for e in completion_events}), "no_mutation_failures": sum(1 for e in events if e.get("event") == "checkpoint_observation" and not e.get("no_mutation")), "latency_s": {"mean": statistics.mean(float(e["latency_s"]) for e in completion_events), "median": statistics.median(float(e["latency_s"]) for e in completion_events), "min": min(float(e["latency_s"]) for e in completion_events), "max": max(float(e["latency_s"]) for e in completion_events)}, "observed_cost_usd": sum(float(e.get("attempt_cost_usd") or 0) for e in completion_events), "status": "CLEAN" if len(terminals) == manifest["logical_calls"] and not any(e.get("error_category") for e in completion_events if e.get("terminal")) else "COMPLETE_WITH_RETRIES"}
